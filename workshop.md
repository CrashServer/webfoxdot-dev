...
- **Let the audience run it too:** each attendee needs their *own local copy* — SharedArrayBuffer needs a secure context (localhost or HTTPS), so a plain `http://<LAN-IP>` won't boot audio. For a shared server, deploy over **HTTPS** (see the nginx block in `README.md`).
