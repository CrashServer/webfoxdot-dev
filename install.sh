...
# ══════════════════════════════════════════════════════════════════════════════
section "STEP 5b — glow (terminal markdown viewer)"
# ══════════════════════════════════════════════════════════════════════════════

explain "glow renders markdown in the terminal with syntax highlighting."
explain "Used by start.sh to display docs (synths, FX, radial menu guide, etc.)."
explain "Optional but recommended."
echo ""

if cmd_exists glow; then
    ok "glow already installed: $(glow --version 2>/dev/null | head -1)"
else
    warn "glow not found."
    echo ""
    if ask_yn "Install glow?" "y"; then
        case "$OS" in
            arch)
                sudo pacman -S --noconfirm glow && ok "glow installed" || fail "glow install failed"
                ;;
            ubuntu|debian)
                info "Adding charm.sh apt repo..."
                ...
                sudo apt-get update -q && sudo apt-get install -y glow \
                    && ok "glow installed" || fail "glow install failed"
                ;;
            macos)
                brew install glow && ok "glow installed" || fail "glow install failed"
                ;;
            *)
                info "Manual install: https://github.com/charmbracelet/glow/releases"
                info "Or: go install github.com/charmbracelet/glow@latest"
                ;;
        esac
    else
        info "Skipped — docs will fall back to less/bat"
    fi
fi
