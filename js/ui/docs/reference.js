...
export const FUNCTIONS = [
    ...
    { name: 'troopclock(on, quantum)', desc: 'Take the beat from webTroop. troopclock(1) matches their tempo and aligns the bar phase; troopclock(1, 8) aligns on 8-beat bars instead of 4; troopclock(0) gives you your own clock back; troopclock() answers with the phase error in beats, which tells you more than yes. Their FoxDot publishes a fractional beat on the telemetry socket, and the correction is the follow-only discipline already used for Ableton Link: a large error snaps, so joining a set in progress catches up on the first reading, and a small one is nudged a fraction per update, so you hear tempo rather than a stumble. Follow-only both ways \u2014 nothing is sent to them, and it is your clock that moves. Needs troop() first and needs their telemetry reachable: watching over localhost gets you their code but no beat. Off by default, because joining a room should not re-time your set. Clock.bpm = linvar(...) writes the tempo every tick too, so a tempo ramp and a followed clock will fight \u2014 pick one' },
