...
export const FUNCTIONS = [
    ...
    { name: 'recall(name|n)',          desc: 'Bring back a saved workspace \u2014 where every panel was, how big, folded or not, and where the canvas was looking. recall(\'performance\') by name (prefix-matched, so recall(\'perf\') finds it) or recall(2) by 0-based index, which WRAPS like theme() and pal= so it can be computed rather than typed. recall() lists them with their numbers. The layouts themselves are saved from the layouts panel; this is the half you can put in a #@ section so the desk changes at the drop' },
