...
export const SHORTCUTS = [
    ...
    { key: 'Ctrl+O',             desc: 'Open a set from a file (or drag one onto the editor)' },
    { key: 'Ctrl+S',             desc: 'Save this buffer as a file · Ctrl+Shift+S for Save As' },
...
export const FUNCTIONS = [
    ...
    { name: 'Server.addFx(opts)',       desc: 'FX over the WHOLE mix \u2014 FoxDot\u2019s Server.addfx. Any effect a player can have works here: Server.addFx(chorus=0.5, crush=6), Server.addFx(fbdelay=0.4, fbtime=0.25), Server.addFx(pumper=1). Calls add up \u2014 a second addFx keeps the first\u2019s effects \u2014 and an effect set to 0 goes dry. Server.clearFx() removes them all; Master.chorus = 0.3 (or Master().chorus = 0.3) sets one knob; Server.addFx() lists what is on. Two places they live: the built-ins of the master synth (gain, ceiling, lpf, hpf, mverb, echo, tanh, drive, lofi) stay there, so sets that already used them sound the same; everything else becomes a node in a master chain after every player and before the limiter, in the same order a player\u2019s effects run. Tempo-synced ones (pong, fbdelay, chop, rgate, pumper) are locked to Clock.bpm for you. The first stop leaves them ringing like any tail; the second, the hard cut, rebuilds them with empty buffers, so a master delay cannot keep sounding after the cut. Values can MOVE: Server.addFx(lpf=linvar([400, 4000], [32])) sweeps the whole mix, and so does any sinvar/var on any master effect \u2014 a plain number afterwards (Master.lpf = 800) stops the sweep rather than fighting it, and Server.addFx() marks the moving ones. Server.removeFx(\'chorus\') takes ONE effect back off \u2014 prefix-matched, so removeFx(\'chor\') finds it, and several at once with removeFx(\'chorus\', \'echo\'). A built-in goes back to its default; a chain effect has its NODE freed, so a master delay stops ringing rather than sitting in the path at zero. Any sweep on that key is dropped first, or the next tick would put the value back. Ctrl+Space inside addFx( lists the built-ins and every player effect; inside removeFx( it lists what is on RIGHT NOW.' },
    { name: 'Server.removeFx(name)',   desc: 'Take one effect off the master bus, leaving the rest \u2014 Server.removeFx(\'chorus\'). clearFx() is all-or-nothing, which mid-set means losing the master filter you spent the whole set building in order to drop one reverb. Names are prefix-matched and you can pass several. A built-in (lpf, echo, mverb\u2026) returns to its default; anything else has its node freed, so a delay\u2019s tail goes with it. Server.removeFx() on its own says what is on. The completion menu inside removeFx( lists exactly what is on the master at that moment.' },
    ...
    { name: 'midi(cc, lo, hi, curve)', desc: 'Bind a MIDI CC knob/fader to a live value — turn it to sweep any param. Use inline or by assignment: p1.lpf = midi(74, 100, 8000, "exp"). curve: lin (default), exp (cutoffs/freq), log, quad, cubic, sqrt, s (smoothstep) — listed in the MIDI panel. Web MIDI (Chromium/Edge); enables on first use A FIFTH argument names the device \u2014 midi(7, 0, 1, "lin", "nano") \u2014 for when two controllers send the same CC numbers, which they do: a nanoKONTROL2\u2019s faders are CC 0..7 and a keyboard\u2019s own volume is CC 7. A name fragment is enough. Without one it answers to that CC from any device, which is what it has always done' },
    { name: 'mlearn(lo, hi, curve)',   desc: 'MIDI learn — binds to the NEXT control you touch, then sticks to it. e.g. p1.amp = mlearn(0, 1). One control can drive several params at once (a macro) It latches onto the control that MOVES, not the first CC that arrives \u2014 a keyboard can sit there restating its state forever and used to win every learn \u2014 and it remembers which box it was on, so the other controller\u2019s CC 7 is not this one\u2019s' },
    ...
    { name: 'midiout(deg, ...)',       desc: 'MIDI OUT — send notes to an external/virtual MIDI port instead of audio: m1 >> midiout([0,2,4], channel=1, oct=5, dur=1). Velocity from amp, note length from sus/leg, groups make chords. Pick the port in the MIDI panel (Chromium/Edge; route via IAC/loopMIDI/ALSA-JACK to a DAW) Settings travel on the same line: prog= (with bank= and banklsb=) picks the instrument, ccNN= sends Control Change NN, and nrpn=[msb, lsb, value] reaches the parameters that have no CC of their own \u2014 all resolved per step like any param, so cc74=sinvar([40, 110], [16]) sweeps a hardware filter, and each is sent only when it CHANGES rather than restated every step' },
    { name: 'midicc(cc, val, ch)',     desc: 'One Control Change to the selected MIDI output \u2014 midicc(74, 100) is brightness, midicc(91, 64) reverb send. Channel defaults to 1' },
    { name: 'midiprog(p, bank, lsb)',  desc: 'Program Change, with Bank Select in front when a bank is given \u2014 midiprog(5, 121, 100). Bank first because a synth latches it when the program arrives' },
    { name: 'midinrpn(msb, lsb, v)',   desc: 'NRPN: CC99/98 address a parameter, CC6 sets it. The settings a keyboard exposes without giving them a CC. A fifth argument sends the fine byte (CC38), which many devices ignore' },
    ...
    { name: 'video knobs (universal)', desc: 'Every layer takes the same set on top of its own params: ch (which deck \u2014 0 or 1), bright, gain, contrast, inv (value), zoom, rot, panx, pany (coords), opacity and blend (how it sits on the layers under it on its own deck), plus pal and dur. speed and scale are the exception: those are the SCENE\u2019s, so a field scene that does not use them cannot be given them, and a workshop layer only has them if it declares its own \u2014 which is why Ctrl+Space offers them for plasma and not for bars or matrixrain. The menu writes only the knobs that actually do something for the name you picked, and never writes one twice: it used to append a universal speed=1 and hue=0 AFTER a layer\u2019s own speed=0.5 and hue=120, and the last one in a call wins, so accepting the completion reset them.' },
    { name: 'perf() / perf("reset")', desc: 'Did the picture cost the audio anything? The engine reports on ITSELF already (health, scheduler lates, drift \u2014 and ?diag logs them every 3s), but it cannot report on the CLOCK, which is main-thread JS: every note is sent about 120ms early carrying a timetag, so scsynth plays it at the right instant no matter how the main thread jitters. A stall shorter than that costs NOTHING; a longer one makes notes miss their timetag. perf() puts both halves together and says which, if either, actually lost. It reports the clock\u2019s p95, p99 and worst tick lateness with a histogram (every tick is sampled, about 100 a second \u2014 the old lag meter took one sample every 700ms and missed most stalls), the count of ticks that ate the whole 120ms of slack, long tasks over 50ms and what they cost, the visuals\u2019 fps and per-frame cost against their budget, and the engine\u2019s own numbers. Then one line of verdict: audio lost nothing, or the main thread ate the slack N times \u2014 in which case vfps(30) or vbudget(2) first, heavy layers after. perf("reset") zeroes the counters so you can measure ONE passage: reset, play the drop, perf().' },
    ...
    { name: 'open / save',             desc: 'A set is a FILE now. [open] or Ctrl+O reads a .py into a new buffer; [save] or Ctrl+S writes the one you are in, and Ctrl+Shift+S is Save As. Where the browser allows it (Chromium) the file is remembered, so saving again writes back to the same one; everywhere else save is a download. Easiest of all: drag a file onto the editor. A set used to live only in localStorage and in a #c= share link \u2014 this browser, this profile, this machine \u2014 so a track written as a .py on disk, which is where people keep them, got in by being pasted. Opening always lands in a NEW buffer: it should never be the thing that loses what you were working on' },
    { name: 'troop(host)',             desc: 'Watch a webTroop session \u2014 the rig the troop plays concerts on. troop(\'192.168.1.38\') with webTroop\u2019s HOST_IP starts it, troop() reports what it is seeing, troop(0) stops. You get their shared buffer with each player\u2019s cursor marked on the line they are editing, their SuperCollider telemetry (bpm \u00b7 cpu \u00b7 beat \u00b7 scale \u00b7 root \u00b7 players), and who is typing what. READ ONLY by construction: the bridge never writes to their document, never sets an awareness field and never sends on any of their sockets, so nothing here can reach a concert in progress \u2014 which is what makes it safe to open during one. If they are running in PRETEXT mode their typing also feeds the visual code layers (livecode, codefull, codeconspiracy), so a troop rehearsal can be the picture. Each of the three feeds reconnects on its own, because a concert rig is routinely half up' },
    { name: 'troopclock(on, quantum)', desc: 'Take the beat from webTroop. troopclock(1) locks to their BEAT NUMBER \u2014 their beat 2742 becomes yours, so bars and counts agree across the room and not just the pulse; troopclock(1, 4) instead aligns only the phase within a 4-beat bar, keeping your own numbering; troopclock(0) gives you your own clock back; troopclock() answers with the phase error in beats, which tells you more than yes. Their FoxDot publishes a fractional beat on the telemetry socket, ten times a second, and the correction splits in two: whole beats are a RENUMBERING, where the clock moves and everything already scheduled moves with it, so nothing sounds different and a fresh join does not dump its backlog into the room; the fraction left over is the part you can hear, eased in at 8% an update. Measured against a live rig it settles inside a thousandth of a beat. Follow-only both ways \u2014 nothing is sent to them, and it is your clock that moves. Needs troop() first and needs their telemetry reachable: watching over localhost gets you their code but no beat. Off by default, because joining a room should not re-time your set. Clock.bpm = linvar(...) writes the tempo every tick too, so a tempo ramp and a followed clock will fight \u2014 pick one' },
    { name: 'troopnudge(ms)',          desc: 'Trim the troop lock BY EAR. troopnudge(12) puts crashDot 12ms later, troopnudge(-8) 8ms earlier, troopnudge(0) back to the raw lock, troopnudge() reads it back \u2014 or the \u2212 / + buttons in the troop panel, 2ms a click. A locked clock is not the same as a lined-up room: their SuperCollider and crashDot\u2019s AudioWorklet empty different output buffers, and the machines are a network hop apart, so the beat can agree to a quarter of a millisecond and still sound a hair out in the air. No number available to either program answers that one, which is why it is a knob and not a calculation. Milliseconds rather than beats, because it corrects a fixed delay \u2014 in beats it would silently change every time somebody moved the tempo' },
    { name: 'compo_base(n, beats)',    desc: 'A skeleton arrangement to write into \u2014 compo_base(16, 4) drops sixteen #@ sections of four beats with a blank line under each. The parts get WORDS rather than part1..part16, because a word tells you something when you are hunting for the section with the breakdown in it, and because in a room with other people you will be saying them out loud. A third argument picks a family and makes the set read as one: compo_base(8, 16, \'birds\') \u2014 cities \u00b7 birds \u00b7 beasts \u00b7 weather \u00b7 stones \u00b7 water \u00b7 machine, or leave it out to mix. Names are distinct by construction, because #@goto resolves to the FIRST section with a name and two parts called the same thing is a set that jumps somewhere you did not mean. Draws from the seeded RNG, so seed(42) gives the same skeleton twice. No #@end \u2014 an arrangement that stops itself is a decision, and this is the part before any have been made' },
    { name: 'recall(name|n)',          desc: 'Bring back a saved workspace \u2014 where every panel was, how big, folded or not, and where the canvas was looking. recall(\'performance\') by name (prefix-matched, so recall(\'perf\') finds it) or recall(2) by 0-based index, which WRAPS like theme() and pal= so it can be computed rather than typed. recall() lists them with their numbers. The layouts themselves are saved from the layouts panel; this is the half you can put in a #@ section so the desk changes at the drop A second argument GLIDES there over that many beats instead of cutting \u2014 recall("drop", 8) moves the panels and the camera together and lands on the beat. Panels that only move are moved; only those that change size are resized, and the editors remeasure once at the end rather than every frame. recall(name) with no time still cuts, which is what you want on a drop' },
    { name: 'store(name)',             desc: 'Save the desk as it stands \u2014 every panel\u2019s position, size, colour and folded state, plus where the canvas was looking \u2014 under a name recall() can bring back. store() lists what is saved; storing an existing name replaces it. The counterpart to recall(), and in code rather than only in the layouts panel, because opening a panel and clicking a row is the one thing you cannot do with your hands on the keyboard mid-set' },
    { name: 'forget(name)',            desc: 'Drop a saved workspace. forget() with no name lists them' },
    { name: 'panel(name, on, x, y)',   desc: 'Open or close any panel from code, and optionally put it somewhere. panel(\'layers\') opens it where it last was \u2014 `on` defaults to 1 \u2014 panel(\'mixer\', 0) closes it, and panel(\'scenes\', 1, 300, 200) opens it at those canvas coordinates. The coordinates are optional on purpose: most of the time \u201cwhere I left it\u201d is the right answer. Takes a name (prefix-matched) or a 0-based index that wraps, and panel() lists everything with its state. The canvas toggles count too \u2014 panel(\'bpm\', 1) puts the tempo and phrase counters on the workspace floor. Which means a section can lay the desk out for the part it is about to play: bring the layers panel up for the video build, drop it again for the breakdown. Desktop mode only \u2014 the classic layout has no panels to place' },
    { name: 'scenes()',                desc: 'Open the SCENE BROWSER \u2014 all 255 visual scenes as live thumbnails, so you can see what one looks like without typing it into a live mix first. Filter by name, click a tile to write its line, shift-click to put it on ch=1. The 49 field scenes render through one shared GL context and the 206 workshop layers draw straight into their own tile; it develops a few tiles at a time, only the ones scrolled into view, so the cost stays flat whether you are showing twelve of them or two hundred' },
    ...
    { name: 'theme(name|n, beats)',    desc: 'Switch the skin. A NAME (prefix-matched, so theme("synth") finds synthwave) or a 0-based INDEX that wraps \u2014 the same shape pal= and blend= already use, so it can be computed rather than typed: theme(n + 1) steps through, theme(PRand(0, 9)) picks a different one each time the line runs, and a negative counts back from the end. theme() with no argument lists them all with their numbers and marks the one you are on. The list is read from the Theme dropdown itself, so the two can never disagree. A second argument FADES there over that many beats instead of cutting: theme(\'paper\', 4) walks every colour in the interface across four beats, theme(1, 2) does the same by number. Beats, not seconds, so a theme change written into a #@ section lands with the bar. It retargets mid-fade \u2014 a second call carries on from the colours currently on screen rather than snapping back \u2014 and picking from the Theme dropdown cuts straight there, whatever is in flight. theme(name) with no time still cuts' },
...
export const PLAYER_PARAMS = [
    ...
    { name: 'sus',      desc: 'Note sustain in beats (defaults to dur). On play() it TRIMS each sample to that length with a 10 ms fade \u2014 play("O", sus=0.25); left out, a sample plays whole' },
    { name: 'cut',      desc: 'FoxDot\u2019s cut: sound for sus \u00d7 cut, then chop in 10 ms. pluck([0, 4], cut=0.5) plays half of each note and drops the synth\u2019s own release, so even a long pad stops dead; play("O", cut=0.25) keeps a quarter of each hit\u2019s slot. 1 or more trims nothing, as in FoxDot. Like every argument it carries over to the next line on the same player \u2014 set cut=1 to take it off' },
