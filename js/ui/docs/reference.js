...
export const FUNCTIONS = [
    ...
    { name: 'Server.addFx(opts)',       desc: 'FX over the WHOLE mix \u2014 everything at once, not one player. Server.addFx(lpf=600, mverb=0.4). Server.clearFx() puts it all back to bypass, and Master.lpf = 800 (or Master().lpf = 800) sets one knob. Server.addFx() with no arguments lists what is there. A FIXED chain, not the 160-key per-player registry \u2014 it is one synthdef sitting at the tail of the node tree reading the summed mix, so it has exactly: gain \u00b7 ceiling \u00b7 lpf \u00b7 lpr \u00b7 hpf \u00b7 hpr \u00b7 mverb \u00b7 echo \u00b7 echo_time \u00b7 echo_dec \u00b7 tanh \u00b7 drive \u00b7 lofi. Anything else (chorus, crush, reverb\u2026) has to go on the players themselves; ask for one here and it says so. Every FX defaults to 0 = bypass, and the limiter after them is always on, which is why a stack of loud players cannot hard-clip.' },
