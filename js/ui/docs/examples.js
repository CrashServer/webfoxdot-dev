...
p1 >> saw([0,3], oct=3, dur=1, lpf_=fb(0.25, 300, 3000))  # wobble`)}
    ...
    const defsynthEx = section('Define synths live — defsynth()', `
        ${note('Build a SynthDef in the browser (no SuperCollider needed) and play it like a built-in. The build fn gets the standard controls (out, note, amp, sus, pan, attack, release) + your extras, each a UGen. Convert pitch with <code>note.midicps()</code>; end with <code>Out.ar(out, …)</code>; use <code>doneAction:2</code> to free the voice. Run the whole example at once if you like — the definition runs first and the lines that play it wait until it has loaded.')}
...
video1 >> boids(count=PRand(60, 240), dur=4)        # …re-rolled every 4 beats, like a player`)}
        ...
        ${note('<b>Maths & attractors</b> — clifford · lorenzsystem · attractor · ifsfractal · burningship · newtonfractal · juliacycles · hyperbolictile · phyllotaxis · spirograph · hitomezashi · truchettiles · floweroflife · polarmandala · magiccircle · runecircle · arabesque')}
        ${note('<b>Audio-reactive</b> — freqtower · waveform · spectrogramscroll · freqmatrix · vumeter · xyoscope · cymatics · chladniplate · audiotterrain')}
        ...
        ${note('<b>Text & typography</b> — text · impacttext · newsticker · mastofeed · propaganda · neonsign · sevensegment · stockticker · scrollingtext · countdown · glitchword · typoburst · terminalprompt · systemboot · codefull · codeconspiracy · livecode · codecomic')}
        ${note('<b>Glitch & film</b> — vhsstatic · glitchstorm · scratchfilm · flickerfilm · filmleader · drostefeedback · pixelsort · fractalkaleidoscope')}
