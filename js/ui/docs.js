...
const CHANGELOG = [
    { v: 'beta01', title: 'Beta! · 3 new themes · offline PWA · zoomable galaxy', items: [
        'Live mixer (experimental) — a fader per track that sets its volume live while a set plays (a persistent per-player gain, _mixLevel, kept SEPARATE from amplify so mute/solo/drop don\'t wipe your mix — final amp = amp × amplify × mixLevel). It\'s in the Players panel per row, and in a pop-out CONSOLE (🎚 mix in the toolbar): a master fader + a channel strip (fader · mute · level) for every track in the composition — the tracks are read from the #@ lines, so they show even before they play. PER-PART levels: the console has a PART PICKER (★Global + a chip per #@ part) — the part playing now is marked ●, pick any chip to edit that part\'s levels, and a • dot marks the tracks that override the global there. So the same d1 can be quiet in the build and loud in the drop, applied automatically whenever a part is entered (auto-advance, tap-to-jump, or evaluating the #@ line). A plain re-eval keeps levels; ~player resets them. LAUNCH from the mixer: tap a track\'s name (▸) to evaluate its line and start it on its own — no need to run the auto-advancing composition, so you can build a mix by hand. It launches the version from the part you\'ve picked (else the part playing, else the first), and since the level is per-name, re-launching keeps the fader where it is. (Per-channel VU meters next — they need per-bus metering.)',
