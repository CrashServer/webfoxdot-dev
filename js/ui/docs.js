...
video1 >> plasma() + sat(1.5) + exposure(1.1) + contrast(1.2) + ceiling(0.85)`)}
        ...
        ${note('<b>Output windows — the projector desk.</b> <code>output()</code> opens a projector window; <code>output(3)</code> gives it three independently warped SURFACES, and that is the point: one output is one projector, and N surfaces in it are N warped patches of content. Pin a flat quad onto each visible face of a box or a truss corner and the object reads as mapped — it is how MadMapper and Resolume handle faceted objects, without any 3D rendering. Each surface picks its own <b>source</b>: the master mix, or any single live workshop layer on its own — so one face can carry the visuals while another carries the <code>codefull</code> layer showing the code that is making them. In the output window: <b>[w]</b> shows the warp handles · <b>[m]</b> cycles 4pt → edge → mesh · <b>[</b> and <b>]</b> change the mesh grid · <b>[r]</b> resets · <b>[f]</b> fullscreen. The OUTPUTS panel does the same from the desktop, with a source picker and edge-blend sliders per surface. Mapping is saved on this machine and never shared or stored in a look — it describes where a projector sits in a room. Outputs are not reopened by themselves after a refresh (a reload that spawns projector windows unasked is worse than one that forgets); REOPEN brings one back with its mapping.')}
        ${note('<b>Projection mapping.</b> In the pop-out visuals window press <b>[m]</b>: four corner handles appear, and dragging them warps the output onto whatever quad your projector actually lands on — the same corner-pin every mapping tool gives you. Four sliders below set <b>edge blending</b>, which fades the output to black along an edge so two projectors can overlap without a bright seam; each one fades its own light where the neighbour begins. <b>[r]</b> resets, <b>[m]</b> again leaves. It is saved on this machine and is deliberately NOT part of a session or a saved look: corners and blends describe where a projector sits in a room, not what the piece looks like, so joining a jam can never yank a calibrated projector.')}
