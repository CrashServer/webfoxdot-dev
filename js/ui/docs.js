...
const CHANGELOG = [
    { v: 'alpha32', title: 'Galaxy map — browse & join live jams', items: [
        '🌌 galaxy (experimental) — a starfield of every live jam session on the server. Each jam is a glowing star: bigger with more people, brighter the more active it is, slowly fading as it goes quiet. Click a star to jump straight into that session. Opens from the toolbar; updates live. When the last person leaves, the jam doesn\'t vanish — it lingers as a dim, slowly-decaying "resting" star (its code kept on the server), so you can click to revive it right where it left off. It\'s purged after a while if nobody comes back.',
    ] },
