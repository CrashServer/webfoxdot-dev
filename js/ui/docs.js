...
const CHANGELOG = [
    { v: 'beta11', title: 'hiss (unified noise) · 5 new voices + tape/bitcrush FX · UX fixes', items: [
        ...
        'Multiplayer now syncs the PERFORMANCE, not just the code — mute, solo, track volume, the mixer\'s ■ stop and Alt+X all reach the room. Until now only text edits, evals, section jumps and eval-level solos crossed the wire: you could pull a fader down or Alt+X a line and your peers would watch the line grey out while still hearing the track at full level, because the comment rides the text sync but the stop it triggers doesn\'t. The hooks sit at the lowest shared layer (gate.js\'s mute/solo, the mixer\'s setLevel/stopPlayer), so every entry path syncs identically — a mixer click, a bound MIDI control, a perform-mode tile drag or hold, the crash panel\'s M/S buttons. Toggles broadcast the RESULTING state rather than the verb, so two people hitting M on the same track at once both land on muted instead of toggling each other back and forth, and fader drags are coalesced to ~20Hz with a trailing send so the room lands on the value you released on. Launching a track from the mixer already synced (it evaluates a line, and evals broadcast), so it wasn\'t touched. Master volume stays LOCAL on purpose — it\'s your own monitoring level, not part of the shared mix.',
        'Multiplayer round 2 — tempo, scale/root, the perform-mode macros, and a shared PANIC. Tempo was the loudest gap: beat_sync already carried bpm in its payload and the receiver threw it away, so the room stayed locked to the same downbeat while running the set at different speeds. The sidebar BPM field and the tap-tempo button now take the room with them, and a follower that missed a change converges on the beat master (skipped while Clock.bpm holds a linvar, or assigning the sampled number would freeze the automation). The crash panel\'s Scale and Root dropdowns share too — typing Scale.default=\'minor\' always synced, since it\'s code; picking it from the menu didn\'t. Perform mode\'s XY pad (filter / space) and the momentary DROP / STUTTER / GATE / ECHO holds now reach everyone, and they land even on a peer who never opened perform mode — they\'re audio state, not a panel. A held FX carries a watchdog: the release is a separate message, so a dropped frame or a peer closing the tab mid-hold would otherwise strand the whole room with the filter slammed shut. Finally, PANIC — stop-all for everyone, on Ctrl+Shift+. , shift-clicking ■ , or panic(). Plain stop-all (■ , Ctrl+; / Ctrl+, / Ctrl+.) stays deliberately LOCAL: it\'s your private escape hatch, and a jam where anyone can silence the room by reflex is worse than one where you have to mean it. Still not synced: a late joiner gets the text but the default mix — actions are fire-and-forget, not stored in the Yjs doc — and a synth built in the modular panel is defined locally only.',
        'Multiplayer round 3 — join a jam in progress and you now arrive INTO it. The mix, tempo, key, pad position and any synth built in the modular panel are shared state living in the session document, not messages relayed once and forgotten, so a peer who turns up mid-set gets the faders where they are rather than the text of the composition with everyone\'s default mix. That split is the whole idea: things that HAPPEN (a stop, a held DROP, a section jump) still ride the relay, while things that ARE (a fader position, the tempo, the key) are state. Keys are per track, so two people riding different faders merge cleanly instead of one clobbering the other, and concurrent edits to the same one converge on a single value. Also closed: a synth you build in the modular panel used to be defined on YOUR machine only, while the p1 >> line the ▸ use it button generates went out to everyone — peers got a player line for a synth they didn\'t have. The generated source travels with it now (shared from the compile step, so live mode\'s auto-redefines carry too, and a late joiner rebuilds every patch the room has defined). Still live-only, so still lost on a late join: which section is playing, and eval-level .solo() — both re-arrive the moment anyone runs something.',
