...
const CHANGELOG = [
    { v: 'beta11', title: 'Multiplayer performs together · hiss (unified noise) · 5 new voices + tape/bitcrush FX', items: [
        ...
        'attack() now SHOWS you what it fired, and the modular panel can hand you its patch as code — attack(\'dresdensunlight\', \'intro\') pastes that part at the end of your buffer, and a trailing 1 — attack(\'sunsetdribble\', \'middle8\', 1) — plays it as well. Showing is the default because dropping a whole set into a live mix unasked is a bigger thing to do by accident than pasting some text, and the flag is what makes attack usable INSIDE a composition: on a #@ section line it both writes the material in and starts it. The paste happens ONCE per block: a #@ section runs again every time the arrangement comes round to it, so a section holding attack(…, 1) would otherwise append the same material on every pass and grow the buffer without limit — pasted blocks are remembered and a repeat call only plays, unless you deleted the text, in which case it comes back. Which means a composition can genuinely pull parts out of OTHER tracks and keep running: a #@ section whose body is attack(\'dresdensunlight\', \'peak\', 1) borrows that peak, writes it into your buffer as a real section you can then edit, and fires it every time your set reaches that point. The autocomplete offers both rows, plain and ▶ play. attack(\'x\', 1) works too — a non-string second argument is read as the flag rather than a part name — a block you cannot read is a black box, and half the reason to call one up is to take it somewhere else. A part keeps its #@ header so what lands is a section you can re-run, jump to and fold, not a loose pile of player lines. It appends rather than inserting at the cursor, so it never drops into the middle of a section or onto a line someone else is editing — and since that puts the material a long way from the attack() line that called it, every pasted block carries a banner naming where it came from, which is also the exact call that fetched it. Without that you were left with anonymous sections at the bottom of the buffer and no way to tell which came from where, or that they had been borrowed at all. And the modular builder has a ⇱ show in code button beside Define: the preview pane always showed the compiled body, but that body is a bare closure, not something you can run — this wraps it into a real defsynth(name, params, fn) with every exposed knob spelled out, so the graph becomes ordinary code you can keep with your set, hand-edit past what the block palette can express, or share, since a composition is only text. It pastes without running, because the reason to press it is to look.',
