...
const CHANGELOG = [
    { v: 'beta11', title: 'hiss (unified noise) · 5 new voices + tape/bitcrush FX · UX fixes', items: [
        'Modular synth builder: knob roles + starter templates. Any unwired knob (an input port, or a Number block\'s value) can now be tagged with a semantic role — click its · badge to cycle freq/amp/rate/custom — so it generates as a mnemonic param (p1 >> mypatch(freq=880)) instead of the auto n2_cutoff name; two knobs tagged with the same role deliberately share one param, and tagging a knob out/note/amp/sus/pan/attack/release reuses that built-in control directly instead of adding a new one. A new "templates ▾" menu (Beep, Buzz lead, Wobble bass, Detuned pad, Noise click) drops in a ready-made patch built from the same block palette, to start from and rewire rather than an empty canvas.',
