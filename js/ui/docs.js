...
export const FUNCTIONS = [
    ...
    { name: 'displace(amt)',           desc: 'Deck B pushes where deck A is READ FROM \u2014 a displacement map. Chain it on a layer: video1 >> plasma() + displace(0.6), with anything on ch=1 as the map. Red moves x and green moves y, the convention every displacement map uses, so a COLOURED layer on B pushes diagonally while a monochrome one pushes along the diagonal only \u2014 which is why the palette on deck B is worth changing when you use this. Nearly free: both decks are already evaluated in the same fragment' },
    { name: 'lumakey(thresh)',         desc: 'Deck A shows through wherever deck B is BRIGHTER than the threshold, and B shows elsewhere. Not a crossfade \u2014 a decision per pixel, with a soft edge so the mask does not alias. Put text, a grid or an ikeda layer on ch=1 and the picture arrives through its bright parts' },
    { name: 'matte(amt)',              desc: 'Deck B\u2019s luminance is deck A\u2019s ALPHA, over black \u2014 B becomes a stencil for A. The companion to lumakey: that one chooses between the decks, this one cuts A out with B and throws the rest away' },
    { name: 'freeze(on)',              desc: 'Hold the frame that is already on screen. The cheapest effect there is \u2014 the scene pass simply does not run \u2014 and the only way to actually LOOK at one. The post-fx and the grade still run over the held frame, so freeze(1) + hueshift(sinvar([0,1],4)) recolours a still. freeze(var([0,1],[7,1])) is a stutter on the beat' },
    { name: 'lut(name|n)',             desc: 'Recolour the FINISHED frame through a palette ramp \u2014 lut(\'fire\') or lut(2), and lutmix(0.4) for a tint rather than a takeover. palette() only ever steered field scenes, because a workshop layer brings its own colour, so until now 206 of the 255 scenes could not be recoloured at all; this is the way in. It reads the frame\u2019s luminance, so it works on anything' },
    { name: 'lutmix(amt)',             desc: 'How much of lut() lands. 1 is a full recolour, 0.4 a tint. Neutral at 1 like the grade keys, not at 0 like the other effects' },
    { name: 'rgbshift(amt)',           desc: 'Chromatic aberration on the whole frame \u2014 red and blue pulled apart radially, strongest at the edges, like a cheap lens. The workshop had this per-layer; this is the finished mix' },
    { name: 'grain(amt)',              desc: 'Film grain over everything. Re-rolled every frame so it reads as grain rather than as texture painted on the picture' },
    { name: 'solarize(amt)',           desc: 'Sabattier: invert everything above the midpoint and leave the shadows alone. Turns a smooth gradient into hard colour bands \u2014 the darkroom accident, on the whole frame' },
    { name: 'threshold(amt)',          desc: 'Hard cut to black and white at the luminance midpoint, with a soft edge so it does not alias. The amount is how far toward the cut, so threshold(0.4) is a strong contrast push and threshold(1) is two-tone' },
    { name: 'tint(hue)',               desc: 'Push the whole frame toward one hue \u2014 0 red, 0.33 green, 0.66 blue, wrapping. Takes a var or a pattern, so tint(sinvar([0,1],16)) walks the picture round the colour wheel. Unlike lut() this is a single colour rather than a ramp' },
    { name: 'halftone(amt)',           desc: 'Print dot screen: the picture rebuilt out of dots whose size follows its brightness. The amount is the dot pitch \u2014 small values are a fine newsprint texture, large ones are a few fat dots' },
    ...
    { name: 'video1.hue = 200',        desc: 'Live-tweak ONE control of a running layer, the same way p1.lpf = 800 does for a player \u2014 no need to re-run the whole >> line. A key the effects table knows (glitch, bloom, trails\u2026) goes to the effect chain, anything else to the layer\u2019s own params, and ch moves it between the two decks. It is the same path the layers panel\u2019s knobs use, so typing the change and dragging it land in exactly the same place. Set before the layer is live, it is held and applied when it arrives' },
    { name: '.every(beats, action)',   desc: 'On a video layer: do something to the PICTURE on the beat grid. The verbs are the ones that mean something to a layer rather than to a note \u2014 video1.every(4, \'ch\') hops it between the decks, video1.every(8, \'scene\', \'plasma\', \'tunnel\') switches scene, video1.every(16, \'reroll\') re-rolls its random params, and anything else is read as a control to cycle: video1.every(4, \'hue\', 0, 180) alternates two values on the grid. On an audio player it is FoxDot\u2019s own .every()' },
    { name: 'video1.reroll(beats)',    desc: 'Re-run this layer\u2019s line every N beats so its random params draw again \u2014 the same .reroll() players have. Under seed(n) it turns a per-line salt, so the line deliberately draws differently while the rest of the set stays reproducible. reroll(0) stops' },
    { name: 'record(...)',             desc: 'Record the whole performance in one gesture. record() starts every recorder that can run \u2014 code (your evals as a #@ composition), audio, MIDI and video \u2014 under ONE take name like crashdot-20260918-2130, and record() again stops them all and saves. record(\'audio\',\'video\') picks. Each of the four still has its own button and still works alone; what this adds is that one performance is one gesture and the files that come out of it say they belong together. Anything already armed by hand is adopted into the take rather than restarted. A recorder that cannot run (no picture on screen, a tab-share dialog dismissed) is skipped and named, and does not stop the others' },
    { name: 'seed(n)',                 desc: 'Fix the randomness, so a set plays the same way every time and every machine in a session plays the SAME notes. seed(42), or seed(\'dresden\') \u2014 a word works and is easier to remember for a set. seed() reports, seed(0) goes back to free (the default). Under a seed every generator draws from (the seed, the line\u2019s own text, which pattern in the line, which step), so re-running a line gives the same music and a peer who joined late is not on a different roll. This is the one that was quietly wrong in a jam: evals travel as source text and each peer runs it, so PRand([0,4,7]) gave a different note sequence on every machine in the room. Nothing extra has to travel \u2014 seed(42) is itself a line of code, so it is already broadcast like everything else. .reroll() still rerolls: it turns a per-line salt, which is what makes \u201csame text, deliberately different\u201d possible under a fixed seed' },
    { name: 'uisize(n)',               desc: 'How big the whole interface is \u2014 text, buttons, panels, the editor and the visuals with them. uisize(1.4) or uisize(140), both work because both are what people type; anything from 10 up is read as a percentage. uisize() reports, uisize(0) or uisize(\'reset\') goes back to 100%, and the range is 70\u2013200%. Also \u2212/+ in Settings and Ctrl+Shift+ + / \u2212 / 0. This is the APP\u2019s own size and is remembered between sessions \u2014 the browser\u2019s Ctrl+/\u2212 page zoom still works too and is untouched. It scales the whole box model rather than just the type, so padding, gaps and chrome keep the proportions the layout was designed with' },
...
const CHANGELOG = [
    { v: 'dev01', title: 'Desktop UI · 190 new visual scenes · virtual piano · scratch buffers', items: [
        'record() — the whole performance in one gesture. crashDot can record four things and each was its own button, its own start, its own stop and its own filename: the evals as a #@ composition, the audio, the MIDI, and the picture. Four gestures for one performance, so the files never lined up, and four unrelated names, so a week later nothing said they were the same take. record() now starts everything that can run, record() again stops them all and saves, and record(\'audio\',\'video\') picks; there is a — record take — button too. They share a take name like crashdot-20260918-2130, which is the point. Nothing is reimplemented: each recorder keeps its own machinery and its own button and still works alone. One already armed by hand is ADOPTED into the take rather than restarted. And one failing cannot take the others down — audio needs a tab-share dialog that can simply be dismissed, and that is not a reason for the take to have no video in it.',
        'Six colour effects the WHOLE frame can have: rgbshift, grain, solarize, threshold, tint and halftone. Which effects existed used to depend on which KIND of scene you picked. The workshop ships 52 per-layer effects and crashDot had 27 whole-frame ones, with only 12 names in both — so a workshop layer could be run through halftone or chromatic aberration and a field scene could not, because a field scene is one shader evaluating a scalar field, not a canvas to draw over. 49 of the 255 scenes were shut out of them entirely and nothing could apply any of them to the finished mix. These six are the ones that are pure per-pixel maths, so they are a few lines in a pass that was already running. Named in crashDot\'s lowercase convention rather than the workshop\'s camelCase — rgbshift, not chromaticAberration — because they sit next to glitch(0.5) in a line.',
        'A video layer\'s setAttr and every were no-ops that returned this. So video1.hue = 200 — the syntax that live-tweaks any audio player — silently did nothing on a layer, with no error to say so; .every() swallowed its arguments the same way, and .reroll() was not defined at all, so it threw. Video layers are players: same name space, same >>, same player list, and the vocabulary should not stop at the first thing you try. setAttr now goes through the same setters the layers panel\'s knobs use, so typing a change and dragging it land in the same place. .every(beats, action) has verbs that mean something to a PICTURE rather than to a note — \'ch\' hops decks, \'scene\' switches, \'reroll\' re-rolls the random params, and anything else is a control to cycle, so video1.every(4, \'hue\', 0, 180) alternates two values on the grid.',
        'Alt+I showed one meaning of a name and hid the rest. The lookup was built with "if not already present", so the first entry found won and every later one was dropped. Twenty-one names carry more than one meaning — attack is both the function that pulls a block out of the examples library and the envelope parameter, release is both giving up a claimed track and the envelope, every is a pattern, a function AND a player method — and you got whichever happened to sit earlier in the file. All senses are shown now, separated. A stale short darker() entry above the fuller darker() / lighter() one is gone as well: the same shape as the theme() duplicate fixed earlier, which is how the inspector came to be explaining a darker() with no mention of lighter().',
        'There is a test suite in the repo. Every check this cycle needed — the transpiler differential, the bus-leak harness, the drag traces, the RNG properties — was a throwaway script in /tmp, and all of them were gone the next time /tmp was cleared. node tests/run.mjs is 61 checks of pure logic with no dependencies and no install (this codebase has no build step, and a suite that needs npm i is a suite that stops being run); node tests/browser.mjs is 11 more that need a real page — the GL programs compiling on a real context, five panels tracking a pointer, the three entry points booting clean. Each check carries the reason it exists, because a test whose reason is forgotten is one nobody dares change. Writing them found a real flaw in the NaN guard: a bad param was silently replaced with a default, so the warning built to name the offending synth and param almost never fired. Absent and bad are different, and it reports now. 243 lines of the live gutter\'s parsing also came out of index.html into js/editor/spans.js so that it could be tested at all — which turned up a THIRD copy of "find the matching bracket" hiding in it.',
        'seed(n), and it fixes something that was quietly wrong in every jam. Evals travel as SOURCE TEXT and each peer runs its own copy, so p1 >> pluck(PRand([0,4,7])) produced a different note sequence on every machine in the room — the beat synced, the code synced, the music not. Fifteen generators here called Math.random() directly, so a saved set played differently every time it loaded too. Now seed(42), or seed("dresden") because a word is easier to remember for a set; seed() reports and seed(0) goes back to free, which stays the default. The interesting part is that it is ADDRESSED rather than sequential. The obvious fix — one seeded PRNG everybody advances together — does not work, because PRand.get() re-rolls on every call and nothing makes two peers call it the same number of times: one machine drops a frame or evaluates a second line in between and the streams part company forever. So a value is hash(seed, the line\'s own text, which pattern in the line, which step), and two peers asking for step 17 agree whatever else either has done. Verified across two independent browser contexts: different sequences unseeded, identical under seed(42). Stateful walks — melody, PWalk, PChain, PBrown — are replayed rather than remembered, so a peer joining mid-set lands on the same point in the walk instead of starting its own at zero. The per-NOTE decisions are addressed too: .degrade() and the .sometimes() family fire per note in each peer\'s own copy of the player, so without this the room came apart bar by bar even with the code shared. Nothing extra travels for any of it, because seed(42) is a line of code and is already broadcast like everything else. .reroll() still rerolls — it turns a per-line salt, which is what makes "same text, deliberately different" possible under a fixed seed.',
        'There is a control for how big the interface is. All 237 font sizes in the stylesheet sit between 8px and 16px, and most are 9, 10 or 11 — a deliberate look, it keeps a lot of state on screen at once, but it assumes the eyes of whoever authored it. On a projector, on a laptop across a room, or for anyone who simply needs text bigger, there was nothing to turn. Now there is: SIZE in Settings (— 100% + reset), Ctrl+Shift+ + / — / 0, and uisize(1.4) in the language. A fraction or a percentage both work, because both are what people type; anything from 10 up is read as a percentage. Range 70—200%, remembered between sessions. It scales the whole box model rather than just the type — padding, gaps, fader widths, panel chrome — so the layout keeps the proportions it was designed with instead of coming apart as the type grows into it, which is what a --ui-scale on every font-size would have done (and would have meant rewriting 237 declarations to find out). The browser\'s own Ctrl+/— page zoom still works and is untouched; this is the app\'s own size, saved with your setup. It composes correctly with the desktop\'s canvas zoom: dragging a panel at 135% tracks the cursor to within 0.2px, because panel.js measures the effective scale off a real element rather than assuming it.',
        'Fixed: desktop panels jittered against the cursor while you dragged them. Measured over a 60-step drag, the panel tracked the cursor exactly on 26 of 60 steps and the error changed DIRECTION 36 times — which is what "it jumps around" is. Every panel, not just the mixer. The cause was the grid pull, and it was not a tuning problem: gridLinesNear() recomputes its candidate lines from the CURRENT position on every move, so there is no fixed lattice to sit between. Each of the three anchors (near edge, far edge, centre) lands within the pull radius of its own nearest line about a third of the time, something is therefore tugging roughly three quarters of the time, and which anchor wins keeps changing as the panel moves. Hysteresis alone made it worse — tried and measured: with a 22px grid and three anchors, any release radius large enough to stop the flip-flopping is also large enough that the panel is always within reach of some line, so it stops moving freely and stair-steps between multiples of 22 instead. So the grid goes back to being what its own comment already called it, a background hint you align to by eye, and the pull is reserved for lines that mean something: the edges, centres and sizes of the OTHER panels. Those stay put while you drag, so there is real space between them and a snap can simply hold until the pointer genuinely leaves it. Exact tracking went from 26/60 to 57/60 steps and the direction reversals from 36 to 2. The resize handle had the same grid pull and lost it too.',
        'Security, and it is the one to take seriously: serve-lan.py served ANY FILE ON THE MACHINE. The /workshop/ mount joined the request path onto a directory and ran os.path.normpath over it — and normpath RESOLVES \'..\' rather than rejecting it, so GET /workshop/../../../.ssh/id_rsa returned the key. Python\'s own translate_path strips \'..\' components for exactly this reason; mounting /workshop/ overrode the method that does it, so the one branch that skipped the sanitiser was also the one taking a path straight from the network. serve-lan.py binds 0.0.0.0, which is its whole purpose — it is the workshop server for the room — so this was reachable by anyone on the venue wifi, reading as whoever started it. The resolved path now has to sit inside the mount or it 404s. serve.py had the identical bug but binds 127.0.0.1, so it was never remotely reachable; fixed there too, since the two are edited in lockstep and one of them is exposed. The same branch also never unquoted the path, so a workshop file with a space in its name 404\'d.',
        'A NaN can no longer silence the room, from any direction. This has been fixed three times — a blown feedback loop, aud()*1 missing from the transpiler\'s pattern list, a cross-player read of a player that had not played yet — and each time it was a real fix to a real SOURCE, and each time it was not the last one. The reason it keeps costing so much is that the sink is unguarded: buildParams is the single funnel every synth note passes through, and it took whatever it was given. `??` catches null and undefined and passes NaN straight on, which is how it kept slipping past. Now every param the builder names goes through a finite check, there is a sweep over the finished array as a backstop (extraParams is open-ended, so a synth added later would not have been covered), and it logs which synth and which param, once, so a bad pattern is findable rather than merely survivable. A non-finite PITCH drops the note rather than being sanitised — zero would sound a very low C on every step of a broken pattern. Why it matters that this is the sink and not another source: a NaN sus means the voice\'s envelope never completes, so doneAction:2 never frees it and it emits NaN forever, and a player with no FX chain sends out=0, straight past the Sanitize on fd_fx_out. Verified byte-identical on 3570 normal-note combinations across all 85 synths.',
        'Fixed: EVERY stop() was orphaning its FX chain and leaking its bus. The deferred teardown — which exists so a note can ring out instead of clicking off — guarded itself with a generation counter, and that counter is only ever initialised in _resetState(), which runs on `~player >>` and nowhere else. A player created the ordinary way never had the field, so ++undefined is NaN, NaN !== NaN is true, and the teardown returned early every single time. Not an edge case: every stop of every player that was never tilde-reset, for as long as this has been in. An FX chain is running synths, so each orphan is permanent DSP on the audio graph and its private bus never returns to the pool — CPU climbing over a long set with nothing in the UI to say why. Measured with 40 players created and stopped: 40 buses never came back before, all 40 recycled after. The check is removed rather than repaired, because it was guarding against something that cannot happen: the chain and bus are captured and cleared in the same breath, so that closure is the only thing that can still reach them.',
        'The shared beat was off by however far apart the two machines\' clocks were. `Date.now() - wallTime` is not elapsed time — it subtracts the master\'s clock from ours, so it measures elapsed PLUS the wall-clock skew, and nothing keeps two laptops in step. Where the follower\'s clock ran BEHIND, the result clamped to zero and looked about right; where it ran AHEAD, the skew became phantom elapsed time. Simulated at four seconds ahead, the room beat lands 7.94 beats out — nearly two bars — and stays there. It never showed up in testing because two tabs on one machine have a skew of exactly zero, which is the shape of test that produced the four-millisecond figure quoted when the shared beat was first added. Everything now converts into the server\'s clock domain first, where the skew cancels. Two related faults went with it: clockOffset had two writers with OPPOSITE signs, so which convention was in force depended on which message arrived last; and electing a beat master reset the offset to zero — true for the FIRST master, false for every successor, so when a master left, the next one re-based the room onto its own boot-relative counter and announced a beat 995 off the one the room was on.',
        'The projection pop-out was missing six effects, and the live gutter had its own parser. Two instances of the same thing: a second copy that drifted. surface.js said in its own header that it was the one copy of fxBundle; render/main.js — the window you actually project from — still had a second, and displace, lumakey, matte, freeze, lut and lutmix worked everywhere except there. And index.html carried its own find-the-matching-bracket and split-the-arguments for the live highlight, with different rules from the transpiler\'s: no string handling at all, and every < and > counted as a bracket. So play("x,o", sample=1) split inside the string, an arrow function closed the argument list early, and a > inside a text= param truncated it. The transpiler was not innocent either — fourteen separate quote-skipping loops, three of which handled a backslash escape and eleven of which did not, so the CONTENTS of a string were rewritten by whichever passes happened to be blind to it. Two of the shipped changelog entries had their own prose corrupted that way. One string scanner now (js/editor/scan.js), one fxBundle (render/fxbundle.js), and one panel-drag helper in place of five identical copies. 1309 lines of real FoxDot from the examples, the tour and the tracks transpile identically; 20,000 randomised layer stacks give identical FX bundles.',
        'Six new video effects, and three of them are a capability rather than a look. The mixer has had two decks from the very beginning and the only thing it could ever do with them was CROSSFADE \u2014 seven blend modes, all of them answering the same question, \u201chow much of B do I see\u201d. displace(amt) reads deck B as DATA for deck A instead: B\u2019s red pushes where A is sampled in x and its green in y, the convention every displacement map uses, which means a coloured layer on B pushes diagonally while a monochrome one pushes only along the diagonal \u2014 the reason the palette on deck B is suddenly worth choosing. lumakey(thresh) shows A wherever B is brighter than the threshold and B elsewhere, with a soft edge so the mask does not alias: put text or a grid on ch=1 and the picture arrives through its bright parts. matte(amt) makes B\u2019s luminance A\u2019s alpha over black, so B becomes a stencil. Together they turn mix() from a fader into a router, and they cost almost nothing \u2014 both decks were already being evaluated in the same fragment; the only new work is evaluating deck A a second time, and only while displace is on.',
        'freeze(on) holds the frame that is already on screen. It is the cheapest effect in the app by a distance \u2014 the scene pass is simply not run \u2014 and the only way to actually LOOK at a frame. The post-fx and the grade still run over the held picture, so freeze(1) + hueshift(sinvar([0,1],4)) recolours a still, and freeze(var([0,1],[7,1])) is a stutter on the beat. lut(name|n) recolours the FINISHED frame through a palette ramp, with lutmix() for a tint rather than a takeover. This one closes a hole that has been open since the workshop was absorbed: palette() only ever steered FIELD scenes, because a workshop layer is an imperative drawing that brings its own colour \u2014 so 206 of the 255 scenes could not be recoloured at all. lut() reads the finished frame\u2019s luminance, so it works on anything, and it takes a palette NAME the way palette() does because \u201cwhich colours\u201d is a question you answer with a word. It is stored 1-based, since the shader needs 0 to mean off and palette index 0 is a real palette. lutmix is neutral at 1 like the grade keys \u2014 the \u201cloudest intent wins from 0\u201d rule every other key uses would have made a partial tint impossible.',
        'Fixed: .follow(p1) threw \u201cp1 is not defined\u201d. Both .follow and .accompany are documented as taking a NAME and both do String(name) on it, so the quoted form has always worked \u2014 but the bare form is what every set in the wild is written with, and it failed as a ReferenceError before the call was even made, which reads as the method being broken rather than the argument being unquoted. A bare identifier there is quoted now. Only a bare identifier: a string, a number, a call or anything with a dot or a bracket in it is left exactly as written, so .follow(nameOf()) and .follow(a.b) still mean what they say.',
        'The eval flash works in SCRATCH buffers now. It was suppressed in every buffer but the set, and the reason was sound for the case it was written for and wrong for everything else: a PEER\u2019s line numbers refer to the shared set, so painting them while you are looking at a scratch would flash whatever happens to sit on those lines \u2014 a different piece of code entirely. But the guard applied to every flash, including your own eval in your own buffer, which is the one case where the numbers are unambiguously right. Your own line always flashes now, wherever you typed it; a peer\u2019s still does not paint a scratch.',
        'And a cross-player read of a player that has not played yet no longer produces NaN. p2 >> saw(p1.degree + 7) evaluated BEFORE the p1 line is ordinary live-coding order \u2014 getAttr resolves to undefined, undefined + 7 is NaN, and a NaN degree or amp poisons the summed bus: the master limiter sanitises it to zero and the mix goes silent until a page refresh. Pmath treats a missing term as contributing nothing now, so the other side comes through unchanged and the result is a number either way. Only undefined, null and actual NaN count as missing \u2014 strings are left alone, so + still concatenates and a named blend or curve still arrives intact. Found by a harness that samples every lazy value in a generated track and looks for non-finite results, which is a better way to find this class than waiting for the room to go quiet.',
        'Fixed: the flash on an evaluated line barely happened. The highlight faded IN over 0.4s while the class was removed at 350ms, so the colour never arrived \u2014 measured, it peaked about 60% of the way there and then faded back down. A flash that takes longer to appear than it lasts is a flash you do not see. It snaps on now and the run pulse\u2019s own half-second animation carries the decay, which is what a blink is supposed to be. (This has been the case since the first commit, so it is not something that regressed \u2014 it just never quite worked.)',
