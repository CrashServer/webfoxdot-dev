...
const CHANGELOG = [
    { v: 'beta01', title: 'Beta! · 3 new themes · offline PWA · zoomable galaxy', items: [
        'Live mixer (experimental) — every active player in the Players panel now has a volume FADER: drag it to set that track\'s level while a set plays, no re-eval. It\'s a persistent per-player gain (_mixLevel) kept SEPARATE from amplify, so mute / solo / drop no longer wipe your mix — fade a track to 0.5, mute and un-mute it, and it returns to 0.5. Composes cleanly: final amp = amp × amplify (mute/solo/drop) × mixLevel (fader). A plain re-eval keeps the level; ~player resets it. Next: a pop-out console with meters + per-section levels.',
