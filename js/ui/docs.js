...
export const FUNCTIONS = [
    ...
    { name: 'ascii_gen(word, style)',   desc: 'Draw a word large in the log, five rows of block characters — a title card for a set, or a marker you can actually find while scrolling. Styles: block · shade · light · hash · dot · star · plus · slash · dash · wave · wide (double-width). Letters, digits and common punctuation; anything unmapped draws a box so a typo is visible rather than silently dropped. Call it with no arguments to list the styles.' },
    { name: 'audiviz(band)',            desc: 'A live level meter in the log, redrawn in place. band: 0 overall level · 1 bass · 2 mid · 3 treble. audiviz(false) stops it. Reads the same analyser the visuals use, so the bar and the picture always agree. Needs audio booted — before that every band reads zero. Also spelled audioviz().' },
    { name: 'theme(name)',              desc: 'Switch the editor skin, or report the current one with no argument. Persists across reloads and mirrors Settings ▸ Theme. One of: dark · nova · cyberpunk · synthwave · hacker · brutalist · paper · sakura · fiesta · solar. Matches on a prefix, so theme("synth") is enough.' },
    ...
    { name: 'parts()',                  desc: 'Open the 🧩 parts panel — every example on the left, its #@ sections on the right, click one to write it into your set. Arm ▶ and a click plays it as well. Filter by block or by part name. It calls attack() for you, so everything true of that is true here.' },
    ...
    { name: 'modular()',                desc: 'Open or close the 🎛 modular synth builder — same as the toolbar button, for when your hands are on the keyboard.' },
...
const CHANGELOG = [
    { v: 'beta11', title: 'Multiplayer performs together · hiss (unified noise) · 5 new voices + tape/bitcrush FX', items: [
        ...
        'A 🧩 parts panel — build a set by clicking, and stop-all finally stops the picture too. The panel puts the whole examples library in two columns: every block on the left grouped by category with a count of the sections it holds, that block\'s #@ parts on the right, and a filter that matches a part NAME as well as a block name — type rumble and you find the track that contains it without knowing which one that is. Clicking writes the part into your set; arm ▶ and it plays as well, off by default because writing is recoverable and dropping a set into a live mix is not. It calls attack() rather than reimplementing it, so the material still lands under your cursor\'s section, is still written in only once, and is still your copy to edit. Separately, a real bug: full stop sometimes left things running. Video players live in the visual language\'s own list rather than the clock\'s, so clearing the clock never touched them and stop-all silenced the audio while the picture carried on — invisible until a set borrows a visuals part, which the new example sets do. A live audiviz meter was missed for the same reason, being a timer rather than a player. Stop everything now means everything.',
