...
const CHANGELOG = [
    { v: 'beta08', title: 'Generative & mixer overhaul — guit audible · chaos evolves every param · live mixer', items: [
        'pbuild — the genre can now be a var / pattern / array, not just a fixed name or index. pbuild(["techno", "house"]) switches genre every bar as the pattern evolves, pbuild([0, 3]) does the same by index, and pbuild(var([...])) follows a TimeVar — the genre resolves per bar and the kit rebuilds only when it changes (a static genre is byte-for-byte unchanged). Before, any non-string/number genre silently fell back to techno. Reminder: dur is a PLAYER param — write play(pbuild("techno", evolve=8), dur=1/16), not dur inside pbuild(...); and layer gates default to ON, so kick=0 mutes and kick=PBin(4) / kick=<1,0> gate (kick=1 is a no-op).',
