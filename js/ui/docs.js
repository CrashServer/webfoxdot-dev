...
#@peak(16)
# FX chained with + fx():  trails · feedback · blur · bloom · scan · vignette · glitch · invert · posterize
video1 >> fire(ch=0, pal="blood", speed=1.5) + trails(0.85) + bloom(0.6)
video2 >> lattice(ch=0, rot=sinvar([-1, 1], [6])) + scan(0.4) + blur(0.3)
video3 >> warp(ch=1, pal="cyber") + vignette(0.5) + feedback(0.7)
...
video3.stop()
video9 >> mix(0)`)}
        ${note('<b>45 scenes</b> — plasma tunnel wave rain spiral cells starfield nebula moire bars grid ripple fire aurora kaleido warp metaballs hexgrid checker swarm flow contour voronoi helix mandala lattice truchet noise rings spectrum marble · testpattern interference biomech escher circuit panopticon penrose mobius hexdump lissajous ikedaglitch barcode equalizer datamatrix. <b>Palettes</b> — fire ice neon sunset matrix mono blood cyber vhs acid. <b>Blends</b> — mix add screen multiply difference wipe dissolve. <b>pal</b> and <b>blend</b> also take a 0-based <b>index</b> (so <code>[]</code> <code>{}</code> patterns work — e.g. <code>pal=[1,7,9]</code>, <code>blend={2,4}</code>). <b>Render modes</b> — smooth pixel shade blocks ascii dots bars. Per-layer knobs — speed scale bright hue pal · zoom rot panx pany · gain contrast inv. <b>FX</b> (chain with <code>+</code>, video players only) — trails feedback blur bloom scan vignette glitch invert posterize droste fold hueshift dither pixelsort mirror edge pixelate.')}
    `, 'vis-tour');
