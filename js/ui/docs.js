...
const CHANGELOG = [
    { v: 'beta09', title: 'Parameter cleanup — one word per idea', items: [
        'Perform mode (phone) scrolls again + gains polish. The tile grid used to hijack every vertical drag as a volume change, so a long set of tracks could never be scrolled — volume is now a HORIZONTAL drag (◄ ►) and a vertical swipe scrolls the grid natively (tap still launches / stops). Added a tempo BEAT-DOT that pulses in the header (accented downbeat), two more momentary FX (GATE · ECHO next to DROP · STUTTER), and a haptic tick on tap. Long-PRESS a tile to SOLO it (shared with the mixer + Players panel — a gold S), and in LANDSCAPE the controls move to a side rail so the tile grid takes the full height.',
